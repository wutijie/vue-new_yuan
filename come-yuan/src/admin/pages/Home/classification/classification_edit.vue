...
<script>
	export default {
		...
		methods: {
			fetchData() {
				fetch("https://wd0227110045vvxhbf.wilddogio.com/classification/"+this.$route.params.id+".json").then(res => {
					console.log(res);
					return res.json();
				}).then(data => {
					console.log(data);
...
//				this.inp_file = event.target.files[0].name;
			...
			handleadd() {
				...
				fetch("https://wd0227110045vvxhbf.wilddogio.com/classification/"+this.$route.params.id+".json",{
					method:"PATCH",
					headers:{
